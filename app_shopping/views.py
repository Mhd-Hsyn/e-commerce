...
class AdminAuthViewset(viewsets.ModelViewSet):
    ...
    def adminSignup(self, request):
        try:
            ...
            requireFeilds_status = uc.requireFeildValidation(
                request.data, requireFeilds
            )
            if requireFeilds_status["status"]:
                ...
                return Response(
                    {"status": False, "message": ser.errors},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            return Response(
                {"status": False, "message": requireFeilds_status["message"]},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            if ser.is_valid():
                ...
                return Response(
                    {
                        "status": False,
                        "message": f"Invalid Credentials {admin_token['message']}",
                    },
                    status=400,
                )
            ...
            if requirefeild_status["status"]:
                ...
                if fetchuser:
                    ...
                    return Response(
                        {
                            "status": True,
                            "message": f"OTP send Successfully check your email {email_to}",
                            "id": str(fetchuser.id),
                        },
                        status=status.HTTP_200_OK,
                    )
                return Response(
                    {"status": False, "error": "No User found in this email"},
        ...
        try:
            ...
            if feild_status["status"]:
                ...
                if fetchuser:
                    ...
                    return Response(
                        {"status": False, "error": "No OTP , Otp Status False"},
                        status=404,
                    )
                return Response(
                    {"status": False, "error": "User not exist"}, status=404
                )
            return Response({"status": False, "error": feild_status["message"]})
        except Exception as e:
            return Response({"status": False, "message": str(e)}, status=400)
        ...
        try:
            ...
            if requiredFeild_status["status"]:
                ...
                if fetchuser:
                    ...
                    return Response(
                        {"status": False, "error": "Token not verified !!!!"}
                    )
...
class AdminViewset(viewsets.ModelViewSet):
    ...
    def adminLogout(self, request):
        try:
            ...
            return Response(
                {"status": False, "error": f"Something wrong {str(e)}"}, status=400
            )
            ...
            if request.method == "PUT":
                ...
                if requireFeilds_status["status"]:
                    (
                        ...
                        fetchuser.last_name,
                        fetchuser.phone,
                    ) = itemgetter("first_name", "last_name", "phone")(request.data)
                ...
                return Response(
                    {"status": False, "error": requireFeilds_status["message"]},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except Exception as e:
            return Response({"status": False, "error": str(e)}, status=400)
            ...
            if feild_status["status"]:
                ...
                if handler.verify(request.data["oldpassword"], fetchuser.password):
                    ...
                    return Response(
                        {
                            "status": False,
                            "error": "New Password Length must be graterthan 8",
                        },
                        status=400,
                    )
                return Response(
                    {"status": False, "error": "Old Password not verified"}, status=400
                )
            return Response(
                {"status": False, "error": feild_status["message"]}, status=400
            )
        except Exception as e:
            return Response({"status": False, "error": str(e)}, status=400)
                ...
                if validator["status"]:
                    ...
                    if ser.is_valid():
                        ...
                        return Response(
                            {
                                "status": True,
                                "message": f"Successfully Added {request.data['name']} Category ",
                            },
                            status=200,
                        )
                    return Response(
                        {"status": False, "error": str(ser.errors)}, status=400
                    )
                return Response(
                    {"status": False, "error": validator["message"]}, status=400
                )
            #  edit Product Category
            elif request.method == "PUT":
                requiredFeilds = ["id", "name", "description"]
                validator = uc.requireFeildValidation(
                    reqData=request.data, requireFeilds=requiredFeilds
                )
                if validator["status"]:
                    ...
                    if fetch_category:
                        ...
                        return Response(
                            {
                                "status": True,
                                "message": f"{fetch_category.name} Category Updated Successfully",
                                "data": ser.data,
                            },
                            status=200,
                        )
                ...
                return Response(
                    {"status": False, "message": f"{validator['message']}"}, status=400
                )
                ...
                if validator["status"]:
                    ...
                    if fetch_category:
                        ...
                        return Response(
                            {
                                "status": True,
                                "message": f"{request.data['id']} Category Deleted !!!",
                            },
                            status=status.HTTP_200_OK,
                        )
                    return Response({"status": False, "message": "Category not exists"})
        ...
        try:
            ...
            if request.method == "GET":
                ...
                if category is not None:
                    ...
                    return Response(
                        {"status": True, "category": category.name, "data": ser.data},
                        status=200,
                    )
                return Response(
                    {"status": False, "error": "Main Category not exist"}, status=404
                )
            ...
            # Post / Add Product Sub Category  needs pk id of main Product category to add in sub-category
                ...
                validator = uc.requireFeildValidation(request.data, requiredFeilds)
                if validator["status"]:
                    ser = AddProductSubCategorySerializer(
                        data=request.data, context={"category_id": pk}
                    )
                    if ser.is_valid():
                        ...
                        return Response(
                            {
                                "status": True,
                                "message": f"{sub_category.name} Successfully added to {sub_category.category.name} Category",
            ...
            # needs pk id of sub category to update the sub category
                ...
                validator = uc.requireFeildValidation(request.data, requiredFeilds)
                if validator["status"]:
                    fetch_category = ProductCategory.objects.filter(
                        id=request.data["category_id"]
                    ).first()
                    fetch_sub_category = Product_SubCategory.objects.filter(
                        id=pk
                    ).first()
                    if fetch_category:
                        if fetch_sub_category:
                            ...
                            return Response(
                                {
                                    "status": True,
                                    "message": f"{fetch_sub_category.name} Successfully changed in {fetch_sub_category.category.name} Category",
                                },
                                status=200,
                            )
                        return Response(
                            {"status": False, "error": f"Sub Category id not exist"},
                            status=404,
                        )
                    return Response(
                        {"status": False, "error": f"Main Category id not exist"},
                        status=404,
                    )
                return Response(
                    {"status": False, "error": f"{validator['message']}"}, status=400
                )
            ...
            # needs pk id of sub category to delete
                ...
                if fetch_sub_category:
                    ...
                    return Response(
                        {
                            "status": True,
                            "message": f"{fetch_sub_category.name} Deleted Successfully from {fetch_sub_category.category.name} Category !!!",
                        },
                        status=200,
                    )
                return Response(
                    {"status": False, "error": f"Sub Category id not exist"}, status=404
                )
        except Exception as e:
            return Response({"status": False, "error": str(e)}, status=400)
